...
export const getApplicationDetails = async (req, res) => {
  try {
    ...
    const application = await Application.findById(applicationId).populate('user', 'name email');
    ...
    res.status(500).json({ message: 'Error retrieving application details', error: error.message });
  }
};
